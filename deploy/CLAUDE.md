...
> ⚠️ **УСТАРЕЛО (2026-05-21).** Docker в проде НЕ используется. Прод работает напрямую:
> PM2 (`pokertitan-server`) + системный Nginx + системный PostgreSQL.
> Актуальные команды сборки/деплоя — в корневом `CLAUDE.md`, раздел «Запуск, сборка, деплой».
> Источник истины для Nginx — `/etc/nginx/sites-enabled/pokertitan` (не файлы в этой папке).
> Команды с `docker compose` ниже оставлены как исторический референс — не выполнять.

## Зона ответственности
Вся инфраструктура: `/deploy`, Nginx-шаблоны, БД, мониторинг.
